...
```{r}
hist(QualiteGenerateur(2), main = "Résultat du test fréquence momobits pour StandardMinimal", xlab = "Pvaleur obtenue", ylab="fréquence", breaks = 100)
```
...
De même que pour StandardMinimal, les Pvaleurs obtenues sont très éparpillées et majoritairement supérieures à 1% (98% des valeurs). Par conséquent, au sens de ce test, nous pouvons affirmer que MersenneTwister génére une séquence aléatoire.

```{r}
hist(QualiteGenerateur(4), main = "Résultat du test fréquence monobits pour la suite de Sobol", xlab = "Pvaleur obtenue", ylab="fréquence", breaks=100)
```
...
En observant l'histogramme, nous apercevons que la quasi-totalité des Pvaleur obtenues sont infèrieures à 0.01. Nous pouvons conclure que les séquences générées ne sont pas aléatoires.

```{r}
hist(testRuns(2), main = "Résultat du test des runs pour StandardMinimal", xlab = "Pvaleur obtenue", ylab="fréquence", breaks=100)
```

Pour StandardMinimal, nous n'avons aucune valeur dépassant les 0.03%. Nous pouvons donc affirmer, sans aucun doute pour ce test, que les séquences générées sont de mauvaise qualité.

```{r}
hist(testRuns(3), main = "Résultat du test des runs pour MersenneTwister", xlab = "Pvaleur obtenue", ylab="fréquence", breaks=100)
```

Nous pouvons apercevoir sur l'histogramme que toutes les Pvaleurs générées sont très inférieures à 0.1%. Pour ce test, nous en déduisons donc que les séquences retournées par MersenneTwister ne sont pas aléatoires bien qu'avec les tests précédents, nous en avons conclut le contraire.
...
Pour le test d'ordre, nous utilisons directement la fonction donnée dans le paquet randtoolbox. Nous testons avec chaque générateur cette fonction, en utilisant le tableau retourné, converti en vecteur, comme paramètre u. Nous effectuons ce test avec une séquence k=1000 et pour 100 initialisations différentes pour chaque générateur (100 valeurs de u différentes dont la graine est générée aléatoirement et différente à chaque fois).

```{r}
hist(TestOrdre(1), main = "Résultat du test d'ordre pour RANDU", xlab = "Pvaleur obtenue", ylab="fréquence", breaks=100)
```

Pour ce premier histogramme, les Pvaleurs obtenues sont très disparates. Nous observons une minorité de valeurs inférieures à 1% (~10%). Nous ne pouvons pas rejeté que les séquences u générées par RANDU sont issus d'une loi uniforme. Donc, pour ce test, nous ne pouvons pas établir d'affirmation concernant le caractère aléatoire des séquences générées par RANDU.

```{r}
hist(TestOrdre(2), main = "Résultat du test d'ordre pour StandardMinimal", xlab = "Pvaleur obtenue", ylab="fréquence", breaks=100)
```

Pareillement que pour le test avec RANDU, les Pvaleurs sont dispersées entre 0 et 1. Nous avons également ~10% des valeurs inférieurs à 1%. Nous pouvons alors effectué la même conclusion que pour RANDU. Nous ne pouvons pas conclure que les séquences u générées par StandardMinimal sont issues d'une loi uniforme. Il n'est donc également pas possible de conclure sur le caractère aléatoire de cette séquence.

```{r}
hist(TestOrdre(3), main = "Résultat du test d'ordre pour MersenneTwister", xlab = "Pvaleur obtenue", ylab="fréquence", breaks=100)
```

Nous pouvons aussi observé ici une grande majorité de Pvaleurs supérieures à 1% (~10%). Comme les deux générateurs précédents, nous ne pouvons pas rejeté l'hypothése que les observations de u sont issues d'une loi uniforme. 
